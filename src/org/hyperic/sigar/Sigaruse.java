...
public class Sigaruse {
	public static void main(String[] args) throws Exception {
		Sigaruse s = new Sigaruse();
	...
	public void getPhysicalMemory() {
		...
		try {
			...
			System.out.println("内存总量："
					+ df.format((float) mem.getTotal() / 1024 / 1024 / 1024)
					+ "G");
			// 当前内存使用量
			System.out.println("当前内存使用量："
					...
					+ "G");
			// 当前内存剩余量
			System.out.println("当前内存剩余量："
					+ df.format((float) mem.getFree() / 1024 / 1024 / 1024)
					+ "G");
			// b)系统页面文件交换区信息
			Swap swap = sigar.getSwap();
			// 交换区总量
			System.out.println("交换区总量："
					+ df.format((float) swap.getTotal() / 1024 / 1024 / 1024)
					+ "G");
			// 当前交换区使用量
			System.out.println("当前交换区使用量："
					+ df.format((float) swap.getUsed() / 1024 / 1024 / 1024)
					+ "G");
			// 当前交换区剩余量
			System.out.println("当前交换区剩余量："
					+ df.format((float) swap.getFree() / 1024 / 1024 / 1024)
					+ "G");
		} catch (SigarException e) {
			e.printStackTrace();
		}
		...
		for (int i = 0; i < fslist.length; i++) {
			...
			case 2: // TYPE_LOCAL_DISK : 本地硬盘
				...
				System.out.println(" Total = "
						+ df.format((float) usage.getTotal() / 1024 / 1024)
						+ "G");
				// 文件系统剩余大小
				System.out.println(" Free = "
						+ df.format((float) usage.getFree() / 1024 / 1024)
						+ "G");
				// 文件系统可用大小
				System.out.println(" Avail = "
						+ df.format((float) usage.getAvail() / 1024 / 1024)
						+ "G");
				// 文件系统已经使用量
				System.out.println(" Used = "
						+ df.format((float) usage.getUsed() / 1024 / 1024)
						+ "G");
